...
Deep convolutional generative adversarial network (DCGAN) is trained to generate airfoils from a random vector input and the steepest descent optimization process  
...
<em>Schematic showing the combined neural network: the generator network and the coefficient of lift network</em>
...
steepest descent optimization using the Newton's method with iterator index $k$ with different initial guesses $\vec{q}^0$.  
$f(\vec{q}) = C_L$  
Newton iteration: $f(\vec{q}^{k+1}) = f(\vec{q}^k) + h\cfrac{\partial f}{\partial \vec{q}}$  
where   
